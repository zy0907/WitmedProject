...
@RequestMapping("/system/hospital/position")
public class PositionController extends BaseController {
	@Autowired
	private PositionTransport positionTransport;
	@Autowired
	...
	public ResponseVO queryPage(@PathVariable("pageNum") Integer pageNum, @PathVariable("pageSize") Integer pageSize,
	                            @RequestBody PositionVO queryVO) throws Exception {
		// 根据查询视图、分页信息创建查询分页视图 QueryPageVO
		QueryPageVO<PositionVO> queryPageVO = new QueryPageVO<>(queryVO, pageNum, pageSize);
		// 进行分页查询获得 PageVO 对象
		PageVO<PositionVO> pageVO = positionTransport.getPage(queryPageVO);
	 ...
	 */
	@PostMapping("/list")
	public ResponseVO queryList(@RequestBody PositionVO queryVO) throws Exception {
		// 进行列表查询，获得视图列表
		List<PositionVO> voList = positionTransport.getList(queryVO);
	...
	public ResponseVO queryById(@PathVariable("id") Long id) throws Exception {
		// 查询获得结果
		PositionVO resultVO = positionTransport.getById(id);
	...
	public ResponseVO queryByCode(@PathVariable("code") String code) throws Exception {
		// 查询获得结果
		PositionVO resultVO = positionTransport.getByCode(code);
	...
	public ResponseVO save(@RequestBody@Validated PositionVO saveVO, @PathVariable("token") String token,
		...
		if (loginAdmin != null) {
			...
			if (!bindingResult.hasErrors()) {
				// 校验通过，校验编码的唯一性
				// 通过编码查询信息
				if (positionTransport.getByCode(saveVO.getCode()) == null) {
					// 该编码未被占用，则可以进行保存
					if (positionTransport.save(saveVO)) {
						// 保存成功
	 ...
	 */
	@PostMapping("/update")
	public ResponseVO update(@RequestBody@Validated PositionVO updateVO, @PathVariable("token") String token,
	                         BindingResult bindingResult) throws Exception {
		// 校验此时用户是否进行登录，首先根据用户所提交的 Token，从 Redis 中获得用户信息
		AdminVO loginAdmin = (AdminVO) redisUtil.findFromRedis(token, AdminVO.class);
		if (loginAdmin != null) {
			...
			if (!bindingResult.hasErrors()) {
				// 校验通过，校验编码的唯一性
				// 通过编码查询信息
				PositionVO validationVO = positionTransport.getByCode(updateVO.getCode());
				if (validationVO == null || validationVO.getId().equals(updateVO.getId())) {
					// 该编码未被占用，则可以进行保存
					if (positionTransport.update(updateVO)) {
						// 保存成功
						return ResponseVO.successResponseVO("保存成功");
					}
					return ResponseVO.errorResponseVO("保存失败");
